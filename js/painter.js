...
class PaintScene extends Scene {
  constructor(game) {
    ...
    this.pencilCursor = new TextShape(0, 0, "✏️", {
      font: "30px monospace",
      color: "white",
    });
  ...
  render() {
    ...
    for (let s of this.strokes) {
      this.drawPainter.colors.stroke(s);
    }
    // If we have an activeStroke (pencil or eraser in progress),
    // draw that as well
    if (this.activeStroke) {
      this.drawPainter.colors.stroke(this.activeStroke);
    }
    super.render();
  }
   ...
   */
  draw() {
    Painter.colors.stroke(stroke);
    // Common setup
    Painter.save();
    Painter.colors.strokeOptions({
      lineCap: "round",
      lineJoin: "round",
    ...
    Painter.effects.setBlendMode(stroke.compositeOp);
    ...
    Painter.lines.moveTo(pts[0].x, pts[0].y);
    ...
    for (let i = 1; i < pts.length; i++) {
      Painter.lines.lineTo(pts[i].x, pts[i].y);
    }
